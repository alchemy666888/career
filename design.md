...
| `OPENAI_API_KEY` or selected AI provider key | Server | AI evaluation and drafting requests. |
...
Owns provider clients, prompt templates, structured output validation, token/cost controls, rate limiting, and safety checks. The AI module must never be called directly from Client Components.
