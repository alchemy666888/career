...
AI Job Search is a pure Next.js application deployable to Vercel. It uses the App Router for frontend routes, React Server Components and Server Actions for data mutations, PostgreSQL for persistence, and spec documents to guide future AI-assisted job-search features.

## 2. Major subsystems

### 2.0 Next.js application layer

The `app/` directory contains the deployable frontend and API surface:

- `app/page.tsx`: landing page and navigation.
- `app/jobs/page.tsx`: PostgreSQL-backed job tracker.
- `app/jobs/new/page.tsx` and `app/jobs/new/actions.ts`: server-action form for creating jobs.
- `app/jobs/[id]/page.tsx`: job detail view.
- `app/api/health/route.ts`: Vercel-friendly health check that verifies database connectivity.
- `app/globals.css`: shared frontend styling.

The `lib/db.ts` module is the database boundary. It uses `@vercel/postgres` in Vercel-like environments and `pg` for local PostgreSQL connections. The `db/schema.sql` file defines the initial `jobs` and `applications` tables.


### 2.1 Future agent command layer
...
### 2.3 Future job search skill layer
...
- PostgreSQL `jobs` and `applications` tables: canonical application pipeline state.
...
- PostgreSQL connection information is supplied through `DATABASE_URL`; Vercel Postgres variables are supported by `@vercel/postgres`.
- `npm run db:migrate` applies the initial schema using `psql`.
- The app exposes `/api/health` to verify runtime database connectivity after deployment.

## 8. Extensibility guidelines for Codex
