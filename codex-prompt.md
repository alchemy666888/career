...
3. Make the app deployable to Vercel using the standard Next.js deployment flow.
4. Configure production and preview secrets through Vercel Cloud environment variables, not committed files.
5. Do not commit real secrets, `.env.local`, production credentials, API keys, tokens, or database passwords.
6. Do not build automatic external job-application submission.
7. Do not fabricate user career claims in AI-generated content.
8. Preserve existing useful code where it fits the architecture, but migrate or remove code that violates the pure Next.js requirement.
...
- Document every required variable and whether it is server-only or public.
...
### Step 6: Implement product workflows
...
4. Environment variables that must be configured in Vercel Cloud.
...
- Keep AI prompts and provider calls server-side.
