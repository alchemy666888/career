...
AI Job Search is a Vercel-ready, pure Next.js application that turns an AI-assisted workflow into a structured career assistant backed by PostgreSQL. The system helps a job seeker build a verified career profile, discover relevant roles, evaluate fit, tailor CVs and cover letters, prepare for interviews, record outcomes, and learn from the job-market feedback loop.
...
1. **Privacy-aware persistence:** Personal profile data, source documents, generated applications, and tracker state are stored in PostgreSQL and deployment environments controlled by the user.
...
- The project must be implemented as a pure Next.js application using the App Router.
- The application must include a user-facing frontend for browsing, creating, and inspecting job records.
- The application must use PostgreSQL as the database of record.
- The application must be ready to deploy to Vercel with documented environment variables and build commands.
...
- **Portability:** The product must run as a pure Next.js application locally and on Vercel, with PostgreSQL as the persistence layer.
...
- A new user can fork the repository, install dependencies, configure PostgreSQL, run the schema migration, start the Next.js app locally, and deploy it to Vercel.
